...
type BookSliceLoaderConfig struct {
	...
	Fetch func(keys []int64) ([][]*gqlmeetup.Book, []error)
...
type BookSliceLoader struct {
	...
	cache map[int64][]*gqlmeetup.Book
...
func (l *BookSliceLoader) Load(key int64) ([]*gqlmeetup.Book, error) {
...
func (l *BookSliceLoader) LoadThunk(key int64) func() ([]*gqlmeetup.Book, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*gqlmeetup.Book, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *BookSliceLoader) LoadAll(keys []int64) ([][]*gqlmeetup.Book, []error) {
	results := make([]func() ([]*gqlmeetup.Book, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	books := make([][]*gqlmeetup.Book, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *BookSliceLoader) LoadAllThunk(keys []int64) func() ([][]*gqlmeetup.Book, []error) {
	results := make([]func() ([]*gqlmeetup.Book, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *BookSliceLoader) Prime(key int64, value []*gqlmeetup.Book) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*gqlmeetup.Book, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *BookSliceLoader) unsafeSet(key int64, value []*gqlmeetup.Book) {
	if l.cache == nil {
		l.cache = map[int64][]*gqlmeetup.Book{}
	}
	l.cache[key] = value
}
